...
import { ProjectState, Dataset, ANNModel, PredictionResult, ClusterResult } from '../types';
...
const ResultsPage: React.FC = () => {
  ...
  useEffect(() => {
    const saved = storageUtils.getProjectState();
    if (saved) {
      try {
  ...
  const hasResults = projectState.results.predictions || projectState.results.clusters;
  ...
  if (!hasResults) {
    return (
      <div className="text-center py-12">
        ...
        <h2 className="text-xl font-semibold text-secondary-900 mb-2">No Results Available</h2>
        <p className="text-secondary-600 mb-6">Train a model or run clustering analysis to see results here.</p>
      ...
      {projectState.results.predictions && projectState.models.ann && (
        ...
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Model Info */}
            <div>
              <h3 className="text-lg font-medium text-secondary-900 mb-4">Model Configuration</h3>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-secondary-600">Task Type:</span>
                  <span className="font-medium">{projectState.models.ann.config.taskType}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-secondary-600">Architecture:</span>
                ...
                <div className="flex justify-between">
                  <span className="text-secondary-600">Activation:</span>
                  <span className="font-medium">{projectState.models.ann.config.activationFunction}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-secondary-600">Learning Rate:</span>
      ...
      {projectState.results.clusters && (
        ...
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Cluster Summary */}
            <div>
              <h3 className="text-lg font-medium text-secondary-900 mb-4">Cluster Summary</h3>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-secondary-600">Total Samples:</span>
                  <span className="font-medium">{projectState.results.clusters.clusters.length}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-secondary-600">Number of Clusters:</span>
                ...
                {projectState.results.clusters.inertia && (
                  <div className="flex justify-between">
                    <span className="text-secondary-600">Inertia:</span>
            ...
            <div>
              <h3 className="text-lg font-medium text-secondary-900 mb-4">Cluster Distribution</h3>
                ...
                {[...new Set(projectState.results.clusters.clusters)].map(cluster => {
                  ...
                  return (
                    <div key={cluster} className="flex items-center justify-between">
                      <span className="text-sm text-secondary-600">Cluster {cluster}:</span>
